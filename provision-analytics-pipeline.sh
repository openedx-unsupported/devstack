...
until docker exec -i mysql.local.edx.org mysql -uroot -se "SELECT EXISTS(SELECT 1 FROM mysql.user WHERE user = 'root')" &> /dev/null
do
  printf "."
  sleep 1
done
...
if [[ ! -z "`docker exec -i mysql.local.edx.org mysql -uroot -se "SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME='edxapp'" 2>&1`" ]];
then
  echo -e "${GREEN}LMS DB exists, skipping lms schema load.${NC}"
else
  echo -e "${GREEN}LMS DB not found, provisioning lms schema.${NC}"
  docker exec -i mysql.local.edx.org mysql -uroot mysql < provision.sql
  ./load-db.sh edxapp
...
docker exec -i mysql.local.edx.org mysql -uroot mysql < provision-analytics-pipeline.sql
