...
then
  ...
  docker-compose exec mysql bash -c 'mysql -uroot mysql' < provision.sql
  ./load-db.sh edxapp
...
docker-compose exec mysql bash -c 'mysql -uroot mysql' < provision-analytics-pipeline.sql
