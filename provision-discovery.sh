...
docker-compose exec -T discovery bash -c 'source /edx/app/discovery/discovery_env && python /edx/app/discovery/discovery/manage.py create_or_update_partner --site-id 1 --site-domain localhost:18381 --code edx --name edX --courses-api-url "http://lms.local.edx.org:18000/api/courses/v1/" --ecommerce-api-url "http://ecommerce.local.edx.org:18130/api/v2/" --organizations-api-url "http://lms.local.edx.org:18000/api/organizations/v0/" --oidc-url-root "http://lms.local.edx.org:18000/oauth2" --oidc-key discovery-key --oidc-secret discovery-secret'
