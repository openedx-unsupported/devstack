...
# and SERVICES could be 'lms', 'ecommerce', 'lms+ecommerce', and so on.
# For example:
#     make dev.up.registrar+studio
...
# such as in:
#      make registrar-up
# Note that prefix-form targets can only apply to one service at a time,
# but you can always pass more than one target to Make at once:
#      make registrar-up studio-up

# Additionally, this file pulls in environment variables from `options.mk` and
# `options.local.mk`, and pulls in extra targets from `compatibility.mk` and `local.mk`,
# which may be in a variety of forms. Running `make help` will only show targets in this
# file.

# Developers: Please keep this file organized by putting new targets under an
# appropriate section header.

.DEFAULT_GOAL := help

.PHONY: analyticspipeline-tests build-courses clean-marketing-sync \
