...
backup: ## Write all data volumes to the host.
	docker run --rm --volumes-from mysql.local.edx.org -v $$(pwd)/.dev/backups:/backup debian:jessie tar zcvf /backup/mysql.tar.gz /var/lib/mysql
	docker run --rm --volumes-from mongo.local.edx.org -v $$(pwd)/.dev/backups:/backup debian:jessie tar zcvf /backup/mongo.tar.gz /data/db
	docker run --rm --volumes-from elasticsearch.local.edx.org -v $$(pwd)/.dev/backups:/backup debian:jessie tar zcvf /backup/elasticsearch.tar.gz /usr/share/elasticsearch/data

restore:  ## Restore all data volumes from the host. WARNING: THIS WILL OVERWRITE ALL EXISTING DATA!
	docker run --rm --volumes-from mysql.local.edx.org -v $$(pwd)/.dev/backups:/backup debian:jessie tar zxvf /backup/mysql.tar.gz
	docker run --rm --volumes-from mongo.local.edx.org -v $$(pwd)/.dev/backups:/backup debian:jessie tar zxvf /backup/mongo.tar.gz
	docker run --rm --volumes-from elasticsearch.local.edx.org -v $$(pwd)/.dev/backups:/backup debian:jessie tar zxvf /backup/elasticsearch.tar.gz
...
ecommerce-shell: ## Run a shell on the ecommerce container
	docker exec -it ecommerce.local.edx.org env TERM=$(TERM) /edx/app/ecommerce/devstack.sh open
...
vnc-passwords: ## Get the VNC passwords for the Chrome and Firefox Selenium containers
	...
	@docker logs firefox.local.edx.org 2>&1 | grep "VNC password" | tail -1
...
analytics-pipeline-shell: ## Run a shell on the analytics pipeline container
	docker exec -it analytics_pipeline.local.edx.org env TERM=$(TERM) /edx/app/analytics_pipeline/devstack.sh open
...
analytics-pipeline-devstack-test: ## Run analytics pipeline tests in travis build
	docker exec -u hadoop -i analytics_pipeline.local.edx.org bash -c 'sudo chown -R hadoop:hadoop /edx/app/analytics_pipeline && source /edx/app/hadoop/.bashrc && make develop-local && make docker-test-acceptance-local ONLY_TESTS=edx.analytics.tasks.tests.acceptance.test_internal_reporting_database && make docker-test-acceptance-local ONLY_TESTS=edx.analytics.tasks.tests.acceptance.test_user_activity'
...
hadoop-application-logs-%: ## View hadoop logs by application Id
	docker exec -it nodemanager.analytics_pipeline.local.edx.org yarn logs -applicationId $*
