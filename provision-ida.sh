...
docker exec -t ${app_name}.local.edx.org  bash -c 'source /edx/app/$1/$1_env && cd /edx/app/$1/$1/ && make requirements' -- "$app_name"

echo -e "${GREEN}Running migrations for ${app_name}...${NC}"
docker exec -t ${app_name}.local.edx.org  bash -c 'source /edx/app/$1/$1_env && cd /edx/app/$1/$1/ && make migrate' -- "$app_name"

echo -e "${GREEN}Creating super-user for ${app_name}...${NC}"
docker exec -t ${app_name}.local.edx.org  bash -c 'source /edx/app/$1/$1_env && echo "from django.contrib.auth import get_user_model; User = get_user_model(); User.objects.create_superuser(\"edx\", \"edx@example.com\", \"edx\") if not User.objects.filter(username=\"edx\").exists() else None" | python /edx/app/$1/$1/manage.py shell' -- "$app_name"
...
docker exec -t ${app_name}.local.edx.org  bash -c 'source /edx/app/$1/$1_env && cd /edx/app/$1/$1/ && make static' -- "$app_name"
